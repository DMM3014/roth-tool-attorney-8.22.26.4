...
def test_default_projection_math(client):
    ...
    assert summary["lifetime_taxes"] == pytest.approx(7765487.89, rel=1e-4)
    assert summary["ending_net_worth"] == pytest.approx(81426835.12, rel=1e-4)
    assert data["legacy"]["after_tax_estate_to_heirs"] == pytest.approx(140350666.22, rel=1e-4)
...
def test_default_strategy_sweep(client):
    ...
    assert 141_000_000 <= ate <= 143_000_000, f"after_tax_estate out of band: {ate}"
